...
with np.load('./MetattackGraph/Metattack_attacked_Cora_graph_adj.npz') as loader:
...
print('\n "Evasion Attack of Pretrained model Accuracy on Metattack attacked Cora graph: {:.4f} " \n'.format(acc))
...
output = model(data)
acc = accuracy(output[data.test_mask], data.y[data.test_mask])
print('Poisoning attacks_Metattack Attacked_In_training Cora Accuracy: {:.4f}'.format(acc))

accuracies.append(acc)
labels_list.append('Metattack Poisoning attack Attacked_In_training Cora Accuracy')
...
plt.title('Comparison of Metattack attacked Cora Model Accuracies', fontsize=14)
